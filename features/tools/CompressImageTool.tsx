...
export function CompressImageTool({ faqs }: { faqs?: { q: string, a: string }[] }) {
      ...
      <div className="space-y-12">
        ...
        <div className="flex justify-center">
          {result ? (
            ...
            >
              {isProcessing ? <Zap className="animate-spin" /> : <ImageIcon size={20} />}
        ...
        <div className="max-w-md mx-auto">
           <Card className="rounded-[2rem] border-primary/10 shadow-xl overflow-hidden bg-muted/30">
        ...
        {isProcessing && <ProgressOverlay isOpen={isProcessing} progress={progress} status={status} title="Compressing Image" message="Please wait while we compress your image..." />}
        {status === 'completed' && result && <ShareResult toolName="compress-image" />}
      </div>
    </ImageToolLayout>
  )
}
